...
source('E:/GoForIT2/Optimizer/nsga2/boundedPolyMutation.R')
source('E:/GoForIT2/Optimizer/nsga2/boundedSBXover.R')
source('E:/GoForIT2/Optimizer/nsga2/crowdingDist4frnt.R')
source('E:/GoForIT2/Optimizer/nsga2/fastNonDominatedSorting.R')
source('E:/GoForIT2/Optimizer/nsga2/tournamentSelection.R')
source('E:/GoForIT2/Optimizer/nsga2/nsga2R6.R')
...
# p1 <- 2 # RVEA
# p2 <- 0 # RVEA
algorithm <- nsga2R6 # mopsocd # mopsopsa
...
# For NSGAII
     s <- algorithm(objectives, varNo=varcnt, objDim=objfn, lowerBounds=lbounds, upperBounds=ubounds,
                popSize=npop, tourSize=2, generations=maxgen, cprob=1, XoverDistIdx=30, mprob=0.2, MuDistIdx=20)

    # if we are here, execution was successful
    cleanup()

    write.table(stored_misfits, file=paste(output_dir,"stored_misfits_nsga2.csv",sep=""), col.names=TRUE, row.names=FALSE, sep=',')
    write.table(stored_param, file=paste(output_dir,"stored_param_nsga2.csv",sep=""), col.names=TRUE, row.names=FALSE, sep=',')
